...
#line 16 "/home/dima/Desktop/coding/coding_seminars/computer_science_4_sem/my_compiler/frontend/src/parser.y"
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // primary
      // assignment_expr
      char dummy2[sizeof (language::Expression_ptr)];
      ...
      // while_stmt
      // print_stmt
      char dummy3[sizeof (language::Statement_ptr)];

      // stmt_list
      char dummy4[sizeof (language::StmtList)];
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_TOK_GREATER = 22,                      // ">"
        S_TOK_LESS_OR_EQ = 23,                   // "<="
        S_TOK_GREATER_OR_EQ = 24,                // ">="
        S_TOK_LEFT_PAREN = 25,                   // "("
        S_TOK_RIGHT_PAREN = 26,                  // ")"
        S_TOK_LEFT_BRACE = 27,                   // "{"
        S_TOK_RIGHT_BRACE = 28,                  // "}"
        S_TOK_SEMICOLON = 29,                    // ";"
        S_TOK_ID = 30,                           // "identifier"
        S_TOK_NUMBER = 31,                       // "number"
        S_YYACCEPT = 32,                         // $accept
        S_program = 33,                          // program
        S_stmt_list = 34,                        // stmt_list
        S_statement = 35,                        // statement
        S_empty_stmt = 36,                       // empty_stmt
        S_block_stmt = 37,                       // block_stmt
        S_38_1 = 38,                             // $@1
        S_assignment_stmt = 39,                  // assignment_stmt
        S_if_stmt = 40,                          // if_stmt
        S_while_stmt = 41,                       // while_stmt
        S_print_stmt = 42,                       // print_stmt
        S_expression = 43,                       // expression
        S_bitwise_op = 44,                       // bitwise_op
        S_equality = 45,                         // equality
        S_relational = 46,                       // relational
        S_add_sub = 47,                          // add_sub
        S_mul_div = 48,                          // mul_div
        S_unary = 49,                            // unary
        S_primary = 50,                          // primary
        S_assignment_expr = 51                   // assignment_expr
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_assignment_expr: // assignment_expr
        value.move< language::Expression_ptr > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_print_stmt: // print_stmt
        value.move< language::Statement_ptr > (std::move (that.value));
        break;

      case symbol_kind::S_stmt_list: // stmt_list
        value.move< language::StmtList > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, language::Expression_ptr&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const language::Expression_ptr& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, language::Statement_ptr&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const language::Statement_ptr& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, language::StmtList&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const language::StmtList& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_primary: // primary
      case symbol_kind::S_assignment_expr: // assignment_expr
        value.template destroy< language::Expression_ptr > ();
        break;
      ...
      case symbol_kind::S_while_stmt: // while_stmt
      case symbol_kind::S_print_stmt: // print_stmt
        value.template destroy< language::Statement_ptr > ();
        break;

      case symbol_kind::S_stmt_list: // stmt_list
        value.template destroy< language::StmtList > ();
        break;
    ...
    parser (language::Lexer* scanner_yyarg, std::unique_ptr<language::Program> &root_yyarg, language::My_parser* my_parser_yyarg);
    virtual ~parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_PREC_IFX (location_type l)
      {
        return symbol_type (token::PREC_IFX, std::move (l));
      }
#else
      static
      symbol_type
      make_PREC_IFX (const location_type& l)
      {
        return symbol_type (token::PREC_IFX, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_TOK_PRINT (location_type l)
      {
      ...
      static
      symbol_type
      make_TOK_PRINT (const location_type& l)
      {
      ...
      static
      symbol_type
      make_TOK_MUL (const location_type& l)
      {
      ...
      static
      symbol_type
      make_TOK_REM_DIV (location_type l)
      {
        return symbol_type (token::TOK_REM_DIV, std::move (l));
      }
#else
      static
      symbol_type
      make_TOK_REM_DIV (const location_type& l)
      {
        return symbol_type (token::TOK_REM_DIV, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_TOK_XOR (const location_type& l)
      {
      ...
      static
      symbol_type
      make_TOK_LESS_OR_EQ (location_type l)
      {
        return symbol_type (token::TOK_LESS_OR_EQ, std::move (l));
      }
#else
      static
      symbol_type
      make_TOK_LESS_OR_EQ (const location_type& l)
      {
        return symbol_type (token::TOK_LESS_OR_EQ, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_TOK_GREATER_OR_EQ (location_type l)
      {
      ...
      static
      symbol_type
      make_TOK_GREATER_OR_EQ (const location_type& l)
      {
      ...
      static
      symbol_type
      make_TOK_LEFT_BRACE (location_type l)
      {
        return symbol_type (token::TOK_LEFT_BRACE, std::move (l));
      }
#else
      static
      symbol_type
      make_TOK_LEFT_BRACE (const location_type& l)
      {
      ...
      static
      symbol_type
      make_TOK_RIGHT_BRACE (location_type l)
      {
        return symbol_type (token::TOK_RIGHT_BRACE, std::move (l));
      }
#else
      static
      symbol_type
      make_TOK_RIGHT_BRACE (const location_type& l)
      {
      ...
      static
      symbol_type
      make_TOK_NUMBER (int v, location_type l)
      {
      ...
      static
      symbol_type
      make_TOK_NUMBER (const int& v, const location_type& l)
      {
